...
namespace ShoppingApp.Data.Concrete.EfCore.Repositories
{
    public class EfCoreProductRepository : EfCoreGenericRepository<Product>, IProductRepository
    {
        ...
        public async Task<List<Product>> GetSearchResultsAdminAsync(string searchString)
        {
            searchString = searchString.ToLower();
            return await ShopAppContext
                .Products
                .Where(p => p.IsApproved == true && (p.Name.ToLower().Contains(searchString) || p.Description.ToLower().Contains(searchString)))
                .ToListAsync();
        }
