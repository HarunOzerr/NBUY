...
namespace ShoppingApp.Web.Controllers
{
    ...
    public class CardController : Controller
    {
        ...
        [HttpPost]
        public IActionResult AddToCard(int productId, int quantity, string productUrl)
        {
            var userId = _userManager.GetUserId(User);
        ...
        public async Task<IActionResult> DeleteForId(int cardItemId)
        {
            var cardItem = await _cardItemManager.GetByIdAsync(cardItemId);
        ...
        private async void SaveOrder(OrderDto orderDto, string userId)
        {
            ...
            order.OrderState = EnumOrderState.Unpaid; //Bunu geçici olarak böyle tanımladık. Aslında buraya ödeme tamamlanmış halde geleceğiz ve buranın unpaid değil completed olmasını sağlayacağız.
        ...
        private bool CardNumberControl(string cardNumber)
        {
             ...
             * Luhn algoritmasını uygulamak için gerekenler.
             */
            #endregion
            ...
            {
                int nextOddNumber = Convert.ToInt32(cardNumber[i]); //sonraki tek sayı
                int nextOvenNumber = Convert.ToInt32(cardNumber[i+1]); //sonraki çift sayı
