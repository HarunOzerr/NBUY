    ...
    public class SqlProductDAL : IProductDAL
    {
        ...
        public List<Product> GetProductsByCategory(int categoryId)
        {
            ...
            using (SqlConnection connection = GetSqlConnection())
            {
                ...
                {
                    connection.Open();
                    string queryString = $"SELECT ProductId, ProductName, UnitPrice, UnitsInStock FROM Products WHERE CategoryID={categoryId}";
                    SqlCommand sqlCommand= new SqlCommand(queryString, connection);
                    SqlDataReader sqlDataReader = sqlCommand.ExecuteReader();
                    if (sqlDataReader.HasRows)
                    {
        ...
        public List<Product> GetProductsCategoryName(string categoryName)
        {
            List<Product> products = new List<Product>();
            using (SqlConnection connection = GetSqlConnection())
            {
                ...
                {
                    connection.Open();
                    string queryString = $"SELECT ProductId, ProductName, UnitPrice, UnitsInStock FROM Products INNER JOIN Categories ON Products.CategoryID = Categories.CategoryID WHERE CategoryName = '{categoryName}'";
                    SqlCommand sqlCommand= new SqlCommand(queryString, connection);
                    SqlDataReader sqlDataReader = sqlCommand.ExecuteReader();
                    if (sqlDataReader.HasRows)
                    {
